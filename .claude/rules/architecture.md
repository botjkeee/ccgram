...
- `herdr_events.py` — herdr push-event-stream plumbing (the only long-lived unix-socket reader). `open_socket_stream(socket_path, subscriptions)` async-generates pushed event dicts (yields a `SUBSCRIBED` sentinel after the ack so the caller reprimes once the subscription is live; a rejected or unintelligible ack raises `OSError` instead of yielding the sentinel, so the reconnect backoff engages rather than treating a dead subscription as live); `translate_event(obj, pane_to_window)` maps a herdr event dict → neutral `MuxEvent`, filtering the firehose to the watched panes/tabs. Injected into `HerdrManager` as `stream_opener` for socket-free unit tests.
- `agent_status_cache.py` — backend-neutral push-updated `window_id → AgentStatus | None` cache (mirrors `vim_state.py`; depends only on `base.AgentStatus`); three states via the `lookup` reader: cold (`(False, None)`, no push yet), warm-negative (`(True, None)`, pushed "no agent" — skip the fallback), warm (`(True, status)`). The event-stream consumer writes it; `observe._native_agent_status` reads it (push-primary) and `event_stream_monitor._dispatch` evicts on `window_died`. Single event-loop thread, so a plain dict (no lock).
- `foreground_cache.py` — backend-neutral, time-bounded (TTL, not push-updated) `window_id → foreground command` memo; bounds `observe.effective_window`'s `foreground()` resolution (two herdr CLI spawns) to once per TTL window instead of every poll tick, for the steady state where a herdr pane's native `pane_current_command` stays empty after its agent exits. Separate from `agent_status_cache` (push, no TTL) by design. Evicted on `window_died` alongside it.
...
- `event_stream_monitor.py` — `EventStreamMonitor`: supervises one `multiplexer.watch_events(bound_window_ids)` subscription; routes `agent_status` events → `agent_status_cache`, `agent_status_unknown` → cache eviction (keeps the poll layer's fallback lookup alive), `window_died` → the existing (idempotent) `_handle_dead_window_notification`. Restarts the stream when the bound set changes (herdr can't add subscriptions to a live connection); `aclosing` + cancellation close the socket deterministically. Module-level `set/get_active_event_stream` for shutdown (mirrors `session_monitor`). Started only on `supports_event_stream` backends.
